...
// Subscribe to room events: Supabase Realtime broadcast if configured, SSE otherwise.
// onReconnect fires after (re)subscribing so the caller can resync history.
export function useRoomEvents(code: string | null, onEvent: (e: RoomEvent) => void, onReconnect: () => void) {
